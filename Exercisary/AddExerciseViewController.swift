...
class AddExerciseViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        button.titleLabel?.text = "upload photo"
        button.tintColor = .white
        button.addTarget(self, action: #selector(imageButtonTapped), for: .touchUpInside)

        // UIImageView에 UIButton 추가
        imageView.addSubview(button)
        
        NotificationCenter.default.addObserver(self, selector: #selector(keyboardUp), name: UIResponder.keyboardWillShowNotification, object: self.memoTextField)
        NotificationCenter.default.addObserver(self, selector: #selector(keyboardDown), name: UIResponder.keyboardWillHideNotification, object: self.memoTextField)
    ...
    @IBAction func SaveButtonTapped(_ sender: Any) {
        let exerciseType = typeTextField.text ?? ""// 사용자 입력에서 운동 종류를 가져옴
        let exerciseTime = formatDuration(duration: timePicker.countDownDuration) ?? "" // 사용자 입력에서 운동 시간을 가져옴
        let memo = memoTextField.text ?? ""// 사용자 입력에서 메모를 가져옴
        let dateString = date ?? ""
        let date = dateFromString(dateString: date, format: "yyyy.MM.dd")
        
        let filtered = ExerciseViewModel.shared.exercises.filter { $0.dateString == dateToString(date: date, format: "yyyy.MM.dd") }
        
        if filtered.isEmpty { // 생성
            ...
            ExerciseViewModel.shared.updateEvent(exercise: exerciseData) { result in
                switch result {
                case .success:
...
extension AddExerciseViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
            if let image = info[UIImagePickerController.InfoKey.originalImage]
                ...
                FirebaseStorageManager.uploadImage(image: image, pathRoot: "user") { url in
                    if let url = url {
                        UserDefaults.standard.set(url.absoluteString, forKey: self.date)
                    }
                }
